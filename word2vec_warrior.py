...
class makeData:
    def __init__(self,file_name,id):
        self.fn_in = [self.make_path(['./warrior-data','{}.xml'.format(file_name[i])]) for i in np.arange(len(file_name))]
        self.fn_in_txt = [self.make_path(['./warrior-data-txt','{}.txt'.format(file_name[i])]) for i in np.arange(len(file_name))]
        self.fn_out = [self.make_path(['./warrior-data-wakati','{}.xml'.format(file_name[i])]) for i in np.arange(len(file_name))]
        #self.fn_model = self.make_path(['./warrior-data-model','word2vec.model'])
        self.fn_model = self.make_path(['./wikipedia-data-model','word2vec.model'])
        self.fn_w2v = [self.make_path(['./warrior-data-pkl','w2v','{}.pickle'.format(file_name[i])]) for i in np.arange(len(file_name))]
        self.fn_tfidf = [self.make_path(['./warrior-data-pkl','tfidf','{}.pickle'.format(file_name[i])]) for i in np.arange(len(file_name))]
        self.id = id
    ...
    def wakati(self,file_name_in,file_name_out):#分かち書きに変換して保存する
        ...
        while line:
            ...
            #result = [x.split('\t')[0] if len(x.split('\t'))>=6 and x.split('\t')[3].find( for x in word]
            result = []

            for x in word:
                xsplits = x.split('\t')

                if len(xsplits)>=6 and (xsplits[3].find('名詞')>=0 or xsplits[3].find('固有名詞')>=0 or xsplits[3].find('形容詞')>=0 or xsplits[3].find('副詞')>=0):
                #if len(xsplits)>=6 and ((xsplits[3].find('名詞')>=0 and xsplits[3].find('固有名詞')<0) or xsplits[3].find('形容詞')>=0 or xsplits[3].find('副詞')>=0):
                #if len(xsplits)>=6 and (xsplits[3].find('形容詞')>=0 or xsplits[3].find('動詞')>=0):
    ...
    def make_dic(self,input_file,dic_file):#辞書データ読み込み,追加,作成,更新
        ...
        for fn in input_file:
            ...
            f_in.close()
        idf = np.log(entence_cnt)-np.log(word_sentence_cnt)+1
